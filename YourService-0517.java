...
public class YourService extends KiboRpcService {
    ...
    // ============================ Auto-aim routines ============================
private static final double TOLERANCE_DEGREES = 30.0; // 容許誤差30度 //20250617更改
private static final double BACKUP_DIST = 0.5; // 假設的後退距離，依需求調整

private boolean isTagWithinTolerance(Mat cnr) {
    
    if (cnr == null || cnr.rows() == 0) return false;

    double centerX = 0, centerY = 0;
    for (int i = 0; i < cnr.rows(); i++) {
        double[] pt = cnr.get(i, 0);
        centerX += pt[0];
        centerY += pt[1];
    }
    centerX /= cnr.rows();
    ...
    double angle = Math.abs(Math.toDegrees(Math.atan2(centerY, centerX)));
...
private Quaternion aimToTag(Point pose, Quaternion q, Mat K, Mat D, int areaId) {
    ...
    if (cnr != null) {
        if (isTagWithinTolerance(cnr)) {
            Log.i(TAG, "aimToTag: tag within tolerance, no aiming needed.");
            return q;  
        } else {
            Log.i(TAG, "aimToTag: tag detected but out of tolerance, auto correcting...");
            Quaternion q1 = autoAimBasicOnce(pose, q, K, D, areaId);
            return q1;
        }
    }
    ...
    int[] backIDs = resultB.markerIds;
    Mat backCnr = null;

    if (backCorners != null) {
        for (int i = 0; i < backIDs.length; i++) {
            if (backIDs[i] == areaId + 100) {
                backCnr = backCorners.get(i);
            }
        }
    }

    if (backCnr == null || !isTagWithinTolerance(backCnr)) {
        Log.i(TAG, "aimToTag: AR-Tag not found or out of tolerance at backup, moving back to original position without aiming.");
        api.moveTo(pose, q, false);
        return q;
    }

    // 備用流程中標籤存在且在容許誤差，進行特殊瞄準
